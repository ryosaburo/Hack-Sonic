...
待機画面左上の「交換所」から購入できます。誘引ルアーはチェックを入れた次の1投で消費されます。星時計と強化リールは購入後も未装備で、交換所の「装備する／外す」から個別に切り替えます。装備中の効果は次のキャストから適用され、装備状態は再読み込み後も保持されます。釣り場情報は購入すると座標を確認でき、海に戻ると範囲が表示されます。
