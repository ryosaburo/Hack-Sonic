...
- 誘引ルアーは消耗品（250pt）。使用を選んだキャストの開始成立時に1個消費し、その1投は必ずスーパーレア以上が掛かる（スーパーレア約73%／伝説約27%。通常の比 8:2 にルアーの倍率 ×2・×3 を掛けたもの）。失敗・途中終了でも消費する。価格は既知の天体を釣ったときの報酬の期待値（約154pt）を上回るように設定し、ルアーでポイントが増え続けないようにする。
- 星時計（制限時間延長）と強化リール（連打・長押しダメージ増加）は永久装備。購入後は未装備で、プレイヤーが釣り画面の誘引ルアーの下にあるチェックボックスから個別に装備／解除する。装備状態は保存され、次のキャストから効果が適用される。両方併用できる。重複購入はできない。
